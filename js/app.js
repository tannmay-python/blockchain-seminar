...
window.APP = (function () {
  ...
  function starfield() {
    ...
    function frame() { ctx.clearRect(0, 0, W, H); dots.forEach(s => { s.a += s.tw; const al = .08 + Math.abs(Math.sin(s.a)) * .22; s.y += s.vy; if (s.y > H) s.y = 0; ctx.fillStyle = `rgba(${s.c},${al})`; ctx.beginPath(); ctx.arc(s.x, s.y, s.r, 0, 6.2832); ctx.fill(); }); requestAnimationFrame(frame); }
